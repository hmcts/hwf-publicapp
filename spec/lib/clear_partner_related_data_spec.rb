RSpec.describe ClearPartnerRelatedData do
  ...
  context 'when married changed' do
    ...
    before {
      ...
      rails_store.write("questions-#{session_id}-#{app_id}-income_kind", income_params.as_json)
      rails_store.write("questions-#{session_id}-#{app_id}-dob", dob_params.as_json)
      rails_store.write("questions-#{session_id}-#{app_id}-personal_detail", name_params.as_json)
    }
    ...
    describe 'clears partner data' do
      it 'income kind' do
        ...
        params = rails_store.read("questions-#{session_id}-#{app_id}-income_kind")
        expect(params['partner']).to be_blank
      end

      it 'partner dob' do
        service.clear_partner_data
        params = rails_store.read("questions-#{session_id}-#{app_id}-dob")
      ...
      it 'partner name' do
        ...
        params = rails_store.read("questions-#{session_id}-#{app_id}-personal_detail")
